...
let cartQueuer: AsyncQueuer<CartEvent> | null;

export const getCartQueuer = () => {
  if (!cartQueuer && typeof window !== "undefined") {
    ...
    cartQueuer = new AsyncQueuer<CartEvent>(processEvent, {
      initialState,
      concurrency: 1,
    });
...
export const enqueueCartMessage = (action: CartEvent) => {
  const queuer = getCartQueuer();
  queuer?.addItem(action);
};

export const getCartQueueState = () => getCartQueuer()?.store.state;
